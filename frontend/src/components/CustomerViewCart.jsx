...
const CustomerViewCart = ({onClose,data,cost,compeleteOrderCost,compeleteOrderData,tableId,orderId}) =>{
  ...
  const [newCompeleteOrderCost,setNewCompeleteOrderCost] = useState(parseFloat(compeleteOrderCost));
  const [newComeleteOrderData,setNewComeleteOrderData] = useState(compeleteOrderData);
  ...
  const getAllPreviousOrderCost = () => {
    ...
    `http://localhost:8080/waitsys/customer/order/showTotalCost?tableId=${parseInt(newTableId)}`,
    {
      method: "GET",
      headers: {
  ...
  const fetchAllCompeleteOrder = () => {
    const response = fetch(
      `http://localhost:8080/waitsys/customer/order/showAllPreviousItems?tableId=${parseInt(newTableId)}`,
      {
        method: "GET",
        headers: {
          "Content-type": "application/json",
    ...
    <List
      ...
      <List
        ...
        renderItem={(item) => (
          ...
          />
        <Statistic title="Total Cost (Include un placed order)(AUD)" value={newCompeleteOrderCost} />
      </>
